...
> **⚠️ Note**: The current project targets .NET 9. If you only have .NET 8 SDK available, you may need to update the project files to target .NET 8 instead by changing `<TargetFramework>net9.0</TargetFramework>` to `<TargetFramework>net8.0</TargetFramework>` in the `.csproj` files.
...
# KbMcpServer__ExecutablePath=../../../../mcp-server-kb-content-fetcher/bin/Debug/net9.0/mcp-server-kb-content-fetcher
...
### 🎯 Testing Queries via UI

Try these natural language queries using VS Code GitHub Copilot Chat or Claude Desktop:
...
**In Claude Desktop:**
```
# Test the KB server directly
"Search for information about Azure Monitor integration"
"What features does Azure Managed Grafana offer?"

# Test the orchestrator
"Tell me about AMG pricing and explain the benefits"
"How does Azure Managed Grafana compare to self-hosted solutions?"
```

### 🔍 Manual MCP Tool Testing

For testing MCP tools directly in supported clients:

```bash
# Use explicit MCP tool calls in VS Code or Claude:
...
**🚫 Server Won't Start**
- Check .NET 9 is installed: `dotnet --version` (or .NET 8 for fallback)
- Verify environment variables are loaded: `echo $AzureOpenAI__ApiKey`
- Check build artifacts exist: `ls src/*/bin/Debug/net*/`
- If using .NET 8, update project files to target `net8.0` instead of `net9.0`
